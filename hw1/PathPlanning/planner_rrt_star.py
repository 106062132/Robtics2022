    ...
    def planning(self, start, goal, extend_len=None, img=None):
        ...
        for it in range(20000):
            ...
            for item in self.ntree:
                if(item != new_node and utils.distance(item, new_node) < 100 and self._check_collision(item, new_node)==False):
                    near_nodes_set.append(item)
            # TODO: Re-Parent & Re-Wire
            for item in near_nodes_set:
                if(self.cost[item]+utils.distance(item, new_node) < self.cost[new_node]):
                    self.ntree[new_node] = item
                    self.cost[new_node] = self.cost[item]+utils.distance(item, new_node)
            for item in near_nodes_set:
                if(self.cost[new_node]+utils.distance(item, new_node) < self.cost[item]):
                    self.ntree[item] = new_node
                    self.cost[item] = self.cost[new_node]+utils.distance(item, new_node)
            # Draw
            if img is not None:
                for n in self.ntree:
